...
| Privacy Policy URL | `https://fuchsia-belief-040.notion.site/OFFMODE-34309a0b0e3e809b965bd62530627431` (`app.json` `expo.ios.privacyPolicyUrl`) |
...
- **개인정보 처리방침을 ASC와 일관된 한글/영문 양면 페이지로 정비** (현재 Notion 단일 페이지)
- **이용약관 페이지 추가** — 현재 로그인 화면 하단에 "서비스 이용약관 및 개인정보 처리방침에 동의" 문구가 있지만 약관 URL이 없음 (`screens/LoginScreen.jsx:70-72`). 약관 페이지 부재는 리젝 리스크 → 별도 이슈로 분리 검토
...
- [ ] **이용약관 URL 부재 처리**: §4 마지막 항목. 본 PR 범위 밖이지만 ASC 제출 전 별도 이슈로 분리할지 결정 필요
