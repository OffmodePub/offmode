...
|---|---|
| Name (Korean) | **오프모드** |
| Subtitle (Korean) | **매일 한 번, 오프라인 미션 인증** (§1-2 추천안 확정) |

> 영문 로케일 추가 시 Name = `offmode`. 첫 출시는 한국어 단일 로케일로 진행 권장.

### 1-2. 부제 / Subtitle (30자 이내)
**확정: "매일 한 번, 오프라인 미션 인증"** (18자)

선정 근거: `매일`·`오프라인`·`미션`을 유지하면서 앱의 핵심 차별점이자 검색어인 **`인증`을 색인에 추가**. 검색 가치가 없는 "하나의"(4자)를 "한 번,"으로 바꿔 자리도 아끼고 리듬도 개선. 부제·앱이름의 단어는 이미 색인되므로 키워드 필드에서는 중복 제거(§1-3).

다른 후보 (참고용 / 다음 버전에 A/B 테스트할 때):

| 후보 | 글자 수 | 톤 |
|---|---|---|
| 매일 하나의 오프라인 미션 | 13 | 직설적 (구 확정안 — `인증` 미포함) |
| 친구와 함께, 매일 오프라인 미션 | 18 | 소셜 포지셔닝 (친구·함께를 부제로) |
| 폰 내려놓고 시작하는 매일 미션 | 16 | 디지털 디톡스 감성 |

### 1-2-a. 프로모션 텍스트 (Promotional Text, 170자 이내)
ASC의 별도 필드. 앱 심사 없이 언제든 교체 가능하므로 시즌/이벤트 문구로 갱신하기 좋다.

```text
스크린 OFF, 일상 ON 🌙 하루에 딱 하나, 오프라인 미션을 사진으로 인증해요. 혼자서는 조용히 루틴을 쌓고, 친구와는 방을 만들어 🔥 리액션으로 응원하고 콕 찔러 재촉해요. 오늘의 미션이 기다리고 있어요.
```

> 약 108자 / 시즌·이벤트 문구로 교체할 여지 60자 확보. 앱 홈 티커("🌿 오늘도 스크린 OFF, 일상 ON")와 톤 일치.

### 1-3. 키워드 (100자, 콤마 구분)
ASC Keywords 필드는 **100자**까지, 콤마 구분. 띄어쓰기 없이 작성하면 글자 수 절약.

```text
챌린지,갓생,습관,루틴,디지털디톡스,자기계발,친구,동기부여,사진,데일리,목표,함께,산책,기록,취미,일상,작심삼일,리추얼,모임,투두,미라클모닝
```

길이 확인: 위 문자열은 **75자**(단어 55 + 콤마 20). 여유 25자 — 출시 후 검색 데이터를 보고 롱테일 키워드를 추가한다.

> 부제 확정안(§1-2)이 `매일`·`오프라인`·`미션`·`인증`을 이미 색인하므로 이 단어들은 키워드 필드에서 제거하고, 확보된 자리에 `산책`(실제 미션 예시·Health&Fitness 정합), `작심삼일`·`리추얼`·`미라클모닝`(한국 습관 앱 고빈도 연관어), `모임`·`투두`(방 소셜/할일 인접 수요)를 추가했다.
>
> (참고: 구 초안의 "약 88자" 표기는 오기였다. 실제 구 키워드는 56자로, 44자를 놀리고 있었다.)
...
```text
📸 스크린 OFF. 일상 ON.
하루에 딱 하나, 오프라인 미션을 사진으로 인증하는 오프모드.
폰만 보던 하루에서 한 걸음 나가보세요.

오프모드는 혼자 또는 친구와 '방'을 만들어 매일 하나의 오프라인 미션을 사진으로 인증하는 앱이에요.

동네 한 바퀴 산책하기, 안 가본 카페 가보기 — 작지만 확실한 미션 하나로 오늘 하루가 어제와 달라져요.
...
· 내가 정한 시간에 오늘의 미션 알림이 도착해요
· 🔥 리액션으로 서로의 미션을 응원해요
· 콕 찌르기로 아직 안 한 친구를 살짝 재촉해요
· 연속 인증을 이어가며 나만의 기록을 쌓아요
· 인증을 거듭할수록 나만의 캐릭터를 꾸밀 아이템이 하나씩 열려요
...
- [x] ~~Subtitle 선택~~ → "매일 한 번, 오프라인 미션 인증" 확정 (`인증` 색인 추가, §1-2)
