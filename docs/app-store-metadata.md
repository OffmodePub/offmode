...
| 신고 기능 | **출시 전 구현 (선결 작업)** |
| 차단·자동 필터 | 다음 버전 후속 작업 |
| 연락처 노출 | Settings > "문의하기" (`SettingsScreen.jsx:296`) |
| **결과 표기 등급** | **12+** |

> **Apple Guideline 1.2 충족 전략**: UGC 앱은 (1) 부적절 콘텐츠 필터, (2) 신고 기능, (3) 차단 기능, (4) 연락처 노출의 4가지 중 최소한의 모더레이션 메커니즘이 있어야 12+ 통과 가능성이 높다. 첫 출시는 **신고 기능 + 연락처** 두 가지로 진입하고, 차단·자동 필터는 다음 버전 후속 작업.
>
> ⚠️ **선결 작업**: ASC 제출 전 신고 기능이 반드시 구현되어 있어야 한다. 미구현 상태로 12+ 표기 시 리젝 위험 매우 큼. 본 PR(#70)과 별개로 신고 기능 이슈를 선행시킨다.
...
- **차단 기능 + 자동 욕설/부적절 콘텐츠 필터** → Guideline 1.2 의 나머지 두 조건 충족, 12+ 등급의 안정성 강화 (다음 버전)
...
- [x] ~~연령 등급 결정~~ → **12+ (신고 기능 선구현 전제)**. 차단·자동 필터는 다음 버전 후속

### 선결 작업 (ASC 제출 전 필수)

- [ ] **콘텐츠 신고 기능 구현** — 별도 이슈로 분리. 본 PR(#70) 머지와 무관하게 출시 전 반드시 완료되어야 함. 미구현 상태로 12+ 제출 시 리젝 위험 큼
