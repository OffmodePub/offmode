...
>
> ⚠️ **선결 작업**: ASC 제출 전 신고 기능이 반드시 구현되어 있어야 한다. 미구현 상태로 12+ 표기 시 리젝 위험 매우 큼. 본 메타데이터 적용과 별개로 신고 기능 이슈([[#71]])를 선행시킨다.
...
- [ ] **콘텐츠 신고 기능 구현** ([[#71]]) — 본 문서 머지와 무관하게 출시 전 반드시 완료되어야 함. 미구현 상태로 12+ 제출 시 리젝 위험 큼
