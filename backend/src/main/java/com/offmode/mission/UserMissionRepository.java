...
public interface UserMissionRepository extends JpaRepository<UserMission, Long> {
    ...
    @Query("SELECT COUNT(um) FROM UserMission um WHERE um.user.id = :userId AND um.status = 'verified' AND um.missionText LIKE %:keyword%")
    long countVerifiedByTextKeyword(@Param("userId") Long userId, @Param("keyword") String keyword);

    // 시간대별 인증 수 (새벽/오후/저녁)
    @Query("SELECT COUNT(um) FROM UserMission um WHERE um.user.id = :userId AND um.status = 'verified' AND HOUR(um.verifiedAt) >= :fromHour AND HOUR(um.verifiedAt) < :toHour")
    long countVerifiedByHourRange(@Param("userId") Long userId, @Param("fromHour") int fromHour, @Param("toHour") int toHour);

    // 연속 달성 계산용: 인증된 미션의 verifiedAt 목록 (오름차순) - Java에서 LocalDate 추출
    @Query("SELECT um.verifiedAt FROM UserMission um WHERE um.user.id = :userId AND um.status = 'verified' ORDER BY um.verifiedAt ASC")
    List<java.time.LocalDateTime> findVerifiedDateTimes(@Param("userId") Long userId);

    // 오늘 미션 + 인증 사진/캡션 포함
    @Query("""
        SELECT new com.offmode.mission.UserMissionDto(
            um.id, um.missionIcon, um.missionText, um.missionCategory,
            um.status, um.assignedAt, um.verifiedAt, v.photoUrl, v.caption
        )
        FROM UserMission um
        LEFT JOIN com.offmode.feed.Verification v ON v.userMission = um
