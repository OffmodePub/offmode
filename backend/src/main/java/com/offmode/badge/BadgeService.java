...
public class BadgeService {
    ...
    public List<BadgeDto> getUserBadges(Long userId) {
        Map<String, UserBadge> earned = userBadgeRepository.findByUserId(userId).stream()
    ...
    private long verifiedByCategory(Long userId, String category) {
        return userMissionRepository.countByUserIdAndStatusAndMissionCategory(userId, "verified", category);
    }
    ...
    private int maxConsecutiveDays(Long userId) {
        List<LocalDate> dates = userMissionRepository.findVerifiedDateTimes(userId)
                .stream()
